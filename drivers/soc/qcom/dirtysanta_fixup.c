...
const char *buf, size_t len)
{
	...
	case 0:
		...
		for(i=0; i<len; ++i) if(!isprint(buf[i])) {
			len=i;
			break;
		}
		...
		/* Most likely `echo LG-H990` and eating the '\n' */
...
#endif
		ptr->lg_model_name[i=len]='\0';
		while(i--!=0) ptr->lg_model_name[i]=buf[i];
#else
		strlcpy(ptr->lg_model_name, buf, len+1);
#endif
