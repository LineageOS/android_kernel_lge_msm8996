 ...
 *
 * $Id: dhd_ip.c 670950 2016-11-18 08:40:28Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#else
				hrtimer_cancel(&tcpack_sup_module->tcpack_info_tbl[i].timer.timer);
#endif /* TCPACK_SUPPRESS_HOLD_HRT */
				if (tcpack_sup_module->tcpack_info_tbl[i].pkt_in_q) {
#ifdef CUSTOMER_HW10
					void* pkt;
					flags = dhd_os_tcpacklock(dhdp);
					pkt = tcpack_sup_module->tcpack_info_tbl[i].pkt_in_q;
					tcpack_sup_module->tcpack_info_tbl[i].pkt_in_q = NULL;
					dhd_os_tcpackunlock(dhdp, flags);
					DHD_TRACE(("%s %d: sending suppressed(%d) ack\n",
						__FUNCTION__, __LINE__, i));
					dhd_sendpkt(dhdp,
						tcpack_sup_module->tcpack_info_tbl[i].ifidx, pkt);
#else
					PKTFREE(dhdp->osh,
...
#else
			tasklet_hrtimer_init(&tcpack_sup_module->tcpack_info_tbl[i].timer,
