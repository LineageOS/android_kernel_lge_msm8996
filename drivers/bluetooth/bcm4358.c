...
static int bcm4358_bluetooth_probe(struct platform_device *pdev)
{
	...
	pr_err("[BT] bcm4358_bluetooth_probe\n");
	...
	if (pdev->dev.of_node) {
		...
		if (IS_ERR(bt_power_pdata->bt_pinctrl)) {
			...
			pr_err("[BT] bcm4358 Succ pinctrl\n");
		}
	}

	rc = gpio_request(bt_power_pdata->bt_gpio_bt_en, "bcm4358_bten_gpio");
    if (rc)
    {
...
static int __init bcm4358_bluetooth_init(void)
{
	...
	pr_err("[BT] bcm4358_bluetooth_init\n");

    ret = platform_driver_register(&bcm4358_bluetooth_platform_driver);
	if (ret) {
		pr_err("[BT] bcm4358_bluetooth_init failed\n");
	}
	return ret;
}
