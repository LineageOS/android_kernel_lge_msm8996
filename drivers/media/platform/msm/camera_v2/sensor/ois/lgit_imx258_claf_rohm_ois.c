...
/*If changed FW, change below FW bin and Checksum information*/
static struct ois_i2c_bin_list LUCY_1202_LGIT_CLAF_ACTUATOR_FIRMWARE_VER_6S_BIN_DATA = {
	.files = 2,
	.entries = {
		{
		...
		{
			.filename = LUCY_1202_LGIT_CLAF_ACTUATOR_FIRMWARE_VER_6S_BIN_2,
			.filesize = 0x01C0,
			.blocks = 1,
			.addrs = {
	...
	.checksum = LUCY_1202_LGIT_CLAF_ACTUATOR_FIRMWARE_VER_6S_CHECKSUM
};

static struct ois_i2c_bin_list LUCY_1206_LGIT_CLAF_ACTUATOR_FIRMWARE_VER_7S_BIN_DATA = {
	.files = 2,
	.entries = {
		{
		...
		{
			.filename = LUCY_1206_LGIT_CLAF_ACTUATOR_FIRMWARE_VER_7S_BIN_2,
			.filesize = 0x01C0,
			.blocks = 1,
			.addrs = {
				{0x0000, 0x01BF, 0x1C00},
...
int lgit_imx258_claf_rohm_ois_bin_download(struct ois_i2c_bin_list bin_list)
{
	int rc = 0;
	//int cnt = 0;
	...
	rc = lgit_imx258_claf_rohm_ois_poll_ready(LIMIT_STATUS_POLLING);
	if (rc < 0) {
...
int lgit_imx258_claf_rohm_ois_init_cmd(int limit)
{
	int rc = 0;
	uint8_t ois_status = 0;
	...
	rc = lgit_imx258_claf_rohm_ois_poll_ready(LIMIT_STATUS_POLLING + limit);
...
int32_t lgit_imx258_claf_rohm_ois_mode(struct msm_ois_ctrl_t *o_ctrl,
...
int lgit_imx258_claf_rohm_ois_calibration(int ver)
{
...
int32_t lgit_imx258_claf_init_set_rohm_ois(struct msm_ois_ctrl_t *o_ctrl,
						   struct msm_ois_set_info_t *set_info)
{
	...
	case OIS_VER_RELEASE:
		...
		lgit_imx258_claf_rohm_ois_init_cmd(LIMIT_OIS_ON_RETRY);
		break;
	case OIS_VER_CALIBRATION:
	case OIS_VER_DEBUG:
		CDBG("%s OIS_VER_DEBUG\n", __func__);
		lgit_imx258_claf_rohm_ois_calibration(ver);
...
int32_t	lgit_imx258_claf_rohm_ois_on(struct msm_ois_ctrl_t *o_ctrl,
					 struct msm_ois_set_info_t *set_info)
{
	int32_t rc = OIS_SUCCESS;
	pr_err("%s:%d Enter\n", __func__, __LINE__);
...
void lgit_imx258_claf_rohm_write_vcm(int16_t nDAC)
{
	if (vcm_check == 1) {
	CDBG("%s Enter\n", __func__);
	CDBG("[CHECK] nDAC: %d\n", nDAC);
...
EXPORT_SYMBOL(lgit_imx258_claf_rohm_write_vcm);

int32_t	lgit_imx258_claf_rohm_ois_off(struct msm_ois_ctrl_t *o_ctrl,
					  struct msm_ois_set_info_t *set_info)
{
	int rc = OIS_SUCCESS;
...
int32_t lgit_imx258_claf_rohm_ois_stat(struct msm_ois_ctrl_t *o_ctrl,
					   struct msm_ois_set_info_t *set_info)
{
...
int32_t lgit_imx258_claf_rohm_ois_move_lens(struct msm_ois_ctrl_t *o_ctrl,
							struct msm_ois_set_info_t *set_info)
{
...
int32_t lgit_imx258_claf_rohm_ois_pwm_mode(struct msm_ois_ctrl_t *o_ctrl,
							struct msm_ois_set_info_t *set_info)
{
	uint8_t mode = 0;
...
void lgit_imx258_claf_rohm_ois_init(struct msm_ois_ctrl_t *msm_ois_t)
{
	lgit_ois_func_tbl.ini_set_ois = lgit_imx258_claf_init_set_rohm_ois;
	lgit_ois_func_tbl.enable_ois = lgit_imx258_claf_rohm_ois_on;
	lgit_ois_func_tbl.disable_ois = lgit_imx258_claf_rohm_ois_off;
	lgit_ois_func_tbl.ois_mode = lgit_imx258_claf_rohm_ois_mode;
	lgit_ois_func_tbl.ois_stat = lgit_imx258_claf_rohm_ois_stat;
	lgit_ois_func_tbl.ois_move_lens = lgit_imx258_claf_rohm_ois_move_lens;
	lgit_ois_func_tbl.ois_pwm_mode = lgit_imx258_claf_rohm_ois_pwm_mode;
	lgit_ois_func_tbl.ois_cur_mode = OIS_MODE_CENTERING_ONLY;
