...
#define LAST_UPDATE "16-10-21, 13M LGIT OLAF OIS bu24235"

/*If changed FW, change below FW bin and Checksum information*/
#define LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_BIN_1 "bu24235_dl_program_Lucy_LGITAct_OPAF_ICG1020S_rev1_S_data1_0x01.bin"
#define LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_BIN_2 "bu24235_dl_program_Lucy_LGITAct_OPAF_ICG1020S_rev1_S_data2_0x01.bin"

#define LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_CHECKSUM	0x0000CD2F
...
/*If changed FW, change below FW bin and Checksum information*/
static struct ois_i2c_bin_list LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER0_BIN_DATA = {
	.files = 2,
	.entries = {
		{
			.filename = LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_BIN_1,
			.filesize = 0x0184,
			.blocks = 1,
			.addrs = {
				{0x0000, 0x0183, 0x0000},
			}
		},
		{
			.filename = LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_BIN_2,
			.filesize = 0x01C0,
			.blocks = 1,
			.addrs = {
	...
	.checksum = LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER_CHECKSUM
};

/*If changed FW, change above FW bin and Checksum information*/


static int lgit_imx258_rohm_ois_poll_ready(int limit)
{
...
int lgit_imx258_rohm_ois_bin_download(struct ois_i2c_bin_list bin_list)
{
	int rc = 0;
	//int cnt = 0;
	...
	rc = lgit_imx258_rohm_ois_poll_ready(LIMIT_STATUS_POLLING);
	if (rc < 0) {
...
int lgit_imx258_rohm_ois_init_cmd(int limit)
{
	int rc = 0;
	uint8_t ois_status = 0;
	...
	rc = lgit_imx258_rohm_ois_poll_ready(LIMIT_STATUS_POLLING + limit);
...
int32_t lgit_imx258_rohm_ois_mode(struct msm_ois_ctrl_t *o_ctrl,
...
int lgit_imx258_rohm_ois_calibration(int ver)
{
...
int32_t lgit_imx258_init_set_rohm_ois(struct msm_ois_ctrl_t *o_ctrl,
						   struct msm_ois_set_info_t *set_info)
{
	...
	switch (cal_ver) {
	case 0x2:
		pr_err("[CHECK] %s: LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER0_BIN_DATA, s\n", __func__);
		rc = lgit_imx258_rohm_ois_bin_download(LUCY_1017_LGIT_OLAF_ACTUATOR_FIRMWARE_VER0_BIN_DATA);
		break;
	default:
	...
	case OIS_VER_RELEASE:
		...
		lgit_imx258_rohm_ois_init_cmd(LIMIT_OIS_ON_RETRY);
		break;
	case OIS_VER_CALIBRATION:
	case OIS_VER_DEBUG:
		CDBG("%s OIS_VER_DEBUG\n", __func__);
		lgit_imx258_rohm_ois_calibration(ver);
...
int32_t	lgit_imx258_rohm_ois_on(struct msm_ois_ctrl_t *o_ctrl,
					 struct msm_ois_set_info_t *set_info)
{
	int32_t rc = OIS_SUCCESS;
	pr_err("%s:%d Enter\n", __func__, __LINE__);
...
void lgit_imx258_rohm_write_vcm(int16_t nDAC)
{
	if (vcm_check == 1) {
	CDBG("%s Enter\n", __func__);
	CDBG("[CHECK] nDAC: %d\n", nDAC);
...
EXPORT_SYMBOL(lgit_imx258_rohm_write_vcm);

int32_t	lgit_imx258_rohm_ois_off(struct msm_ois_ctrl_t *o_ctrl,
					  struct msm_ois_set_info_t *set_info)
{
	int rc = OIS_SUCCESS;
...
int32_t lgit_imx258_rohm_ois_stat(struct msm_ois_ctrl_t *o_ctrl,
					   struct msm_ois_set_info_t *set_info)
{
...
int32_t lgit_imx258_rohm_ois_move_lens(struct msm_ois_ctrl_t *o_ctrl,
							struct msm_ois_set_info_t *set_info)
{
...
int32_t lgit_imx258_rohm_ois_pwm_mode(struct msm_ois_ctrl_t *o_ctrl,
							struct msm_ois_set_info_t *set_info)
{
	uint8_t mode = 0;
...
void lgit_imx258_rohm_ois_init(struct msm_ois_ctrl_t *msm_ois_t)
{
	lgit_ois_func_tbl.ini_set_ois = lgit_imx258_init_set_rohm_ois;
	lgit_ois_func_tbl.enable_ois = lgit_imx258_rohm_ois_on;
	lgit_ois_func_tbl.disable_ois = lgit_imx258_rohm_ois_off;
	lgit_ois_func_tbl.ois_mode = lgit_imx258_rohm_ois_mode;
	lgit_ois_func_tbl.ois_stat = lgit_imx258_rohm_ois_stat;
	lgit_ois_func_tbl.ois_move_lens = lgit_imx258_rohm_ois_move_lens;
	lgit_ois_func_tbl.ois_pwm_mode = lgit_imx258_rohm_ois_pwm_mode;
	lgit_ois_func_tbl.ois_cur_mode = OIS_MODE_CENTERING_ONLY;
