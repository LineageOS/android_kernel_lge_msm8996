...
static struct security_operations pfk_ecryptfs_security_ops = {
	.name			= "pfk_ecryptf",

	.inode_alloc_security	= pfk_ecryptfs_inode_alloc_security,
	.inode_free_security	= pfk_ecryptfs_inode_free_security,
};
...
static int __init pfk_ecryptfs_lsm_init(void)
{
	...
	if (security_module_enable(&pfk_ecryptfs_security_ops)) {
		/* replace null callbacks with empty callbacks */
		security_fixup_ops(&pfk_ecryptfs_security_ops);
		ret = register_security(&pfk_ecryptfs_security_ops);
		if (ret != 0) {
			pr_err("pfk_ecryptfs lsm registeration failed, ret=%d.\n", ret);
			return ret;
		}
